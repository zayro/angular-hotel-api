...
// and get result in a callback function.
//
function queryDelete(data, callback) {
  var q = buildDeleteQuery(data);
  queryOne(q.sql, q.values, callback);
}
...
// and get result in a callback function.
//
function max(table, field, callback) {    
   // queryOne("select max($2) from $1", [table, field], callback);
    query(`select max(${field}) + 1 as ${field}  from ${table}`,  callback);
}
...
function buildDeleteQuery(data) {
  var sql = 'DELETE FROM ' + data.table;
  var where  = Object.keys(data.where);
  var values = [];
